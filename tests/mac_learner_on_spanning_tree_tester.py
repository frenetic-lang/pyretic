...
def main():
    ...
    # STARTUP CONTROLLER
    controller = Popen([sys.executable, '-u', '-O', pox_path,'--no-cli', 'pyretic/examples/virtualize.py', '--program=pyretic/examples/mac_learner.py', '--virttopo=pyretic/virttopos/spanning_tree_vdef.py'], 
                       env=env,
                       stdout=PIPE, 
                       stderr=STDOUT)
    controller_out = subprocess_output(controller, options.verbose)
    controller_out.start()
    sleep(1)

    # TEST EACH TOPO
    topos = ['single,2','single,16','linear,2','linear,8','tree,2,2','tree,3,2']
#    topos = ['cycle,4,4','clique,4,4']  # THESE TWO AREN'T WORKING CURRENTLY

    print "=== LEARNING SWITCH ON SPANNING TREE TESTER ==="
